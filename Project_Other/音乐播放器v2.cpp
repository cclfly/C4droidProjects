...
void DisplayThread(AudioPlayer& ap,const vector<entity>& audio_list,atomic_int& page,atomic_int& current_music,atomic_int& mode)
{
    int t=0;
    while(!quit_flag)
    {
        ...
        cout<<"Playing:"<<((current_music<0)?"":audio_list[current_music].file)<<endl;       
        cout<<"Time: ";
        ShowProgressBar(((double)current/end)*20,20);
        cout<<current/60<<":"<<current%60<<"/"<<end/60<<":"<<end%60<<endl;
        cout<<"Mode:"<<((mode==ORDER)?"order":"random")<<endl<<endl;
        ...
        cout<<"[P]暂停/播放  "<<"[S]停止播放  "<<"[F]上一页  "<<"[N]下一页  "<<"[0-9]选曲  "<<"[M]模式  "<<"[Q]退出"<<endl;
        
        if(key_down_pause==false&&ap.getState()==SL_PLAYSTATE_PAUSED&&current_music>=0)
        {
            if(mode==ORDER)
            {
                ++current_music;
                current_music=current_music%audio_list.size();
            }
            if(mode==RANDOM)
                current_music=rand()%audio_list.size();
            ap.setSource(audio_list[current_music].path+"/"+audio_list[current_music].file);
            ap.play();
        }
        usleep(250000);
    }
...
int main()
{
	...
	thread display_thread(DisplayThread,std::ref(ap),std::ref(audio_list),std::ref(page),std::ref(current_music),std::ref(play_mode));
